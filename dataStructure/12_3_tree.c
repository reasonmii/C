...
bool AddItem(const Item* pi, Tree* ptree) {
	...
	// MakeNode : left, right 가 NULL인 node 생성
...
static void DeleteAllNodes(Node* root) {
	...
	// right subtree는 지우기 전에 미리 백업 필요
	// -> 마지막에 right subtree 부분도 모두 삭제 가능
...
static Node* MakeNode(const Item* pi) {
	...
	if (new_node != NULL) {
		...
		// 요즘 코딩에서는 이 부분이 없는 경우 통합 개발환경이
		// warning을 발생시켜, 반드시 이 부분을 코딩하도록 함
...
static void DeleteNode(Node** ptr) {
	...
	// wHEN deleted node has two children
		...
		// temp->right 값이 NULL이면 for loop break
